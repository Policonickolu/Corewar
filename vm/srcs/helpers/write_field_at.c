...
/*   Updated: 2019/12/03 11:36:12 by hben-yah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_val(int addr, unsigned char val, int number, int rev)
{
	...
	if (rev)
	{
		mvaddch(y, x++, g_base[val / 16] | A_REVERSE | COLOR_PAIR(((number - 1) % 7) + 1));
		mvaddch(y, x++, g_base[val % 16] | A_REVERSE | COLOR_PAIR(((number - 1) % 7) + 1));
	}
	else
	{
		mvaddch(y, x++, g_base[val / 16] | COLOR_PAIR(((number - 1) % 7) + 1));
		mvaddch(y, x++, g_base[val % 16] | COLOR_PAIR(((number - 1) % 7) + 1));
	}
	//refresh();
	usleep(50000);
